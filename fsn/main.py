...
def handle_posted(driver: Driver, data, channels):
    ...
    if re.match(r"^join [a-z0-9]{6} [0-9]{5}$", command) is None and command not in ["subscribe", "unsubscribe"]:
...
```
subscribe - start being added to new course channels (if you've previously unsubscribed)
unsubscribe - stop being added to new course channels
```
""")
        return

    if command == "subscribe":
        with open(os.path.join(os.path.dirname(__file__), "added_to_channel", "unsubscribed.txt"), "w+") as f:
            u = f.read()
